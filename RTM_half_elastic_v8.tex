...
z^{1/2}={\rm sgn}(z_2)\sqrt{\frac{|z|+z_1}{2}}+\i\sqrt{\frac{|z|-z_1}{2}},\ \ \forall z\in\C\backslash{\R}_+.
\ee
...
\ben
\hskip-1cm\N(x,y)=\frac{1}{2\pi}\,{\rm p.v.}\int_{\R}\hat \N(\xi,x_2;y_2) e^{\i(x_1-y_1)\xi} d\xi+\frac\i 2
\left[\frac{\hat\N_c(\xi,x_2;y_2)}{\de'(\xi)}e^{\i(x_1-y_1)\xi}\right]^{k_R}_{-k_R},
\een
where $[f(\xi)]^b_a:=f(b)-f(a)$, and
\ben
\hat\N_c(\xi,x_2;y_2)=\frac{\i}{\omega^2}\sum_{\alpha,\beta=p,s}\mathbb{A}_{\al\beta}(\xi)e^{\i(\mu_\al x_2+\mu_\beta y_2)}.
\een
...
=\frac 1{2\pi}\,{\rm p.v.}\int_{L_{-\phi}}\mathcal{N}(t)\cos t\,e^{\i\lam\cos t}dt,
\ee
...
\ben
\hskip-1cm\frac 1{2\pi}\,{\rm p.v.}\int_{L_{-\phi}}\mathcal{N}(t)\cos t\,e^{\i \lam\cos t}dt
&=&\lim_{\eps\to 0^+}\frac 1{2\pi}\int_{L^\eps_{-\phi}}\mathcal{N}(t)\cos t\,e^{\i \lam\cos t}dt\\
\hskip-1cm& &+\frac\i 2\sum_{t'=\pm t_R}{\rm Res}(\mathcal{N}(t)\cos te^{\i \lam\cos t},t').
\een
It is easy to see that the residue
\ben
\sum_{t'=\pm t_R}{\rm Res}(\mathcal{N}(t)\cos te^{\i \lam\cos t},t')=\frac \i 2\sum_{\xi=\pm k_R}\sum_{\al=p,s}\frac{\Na(\xi)}{\de'(\xi)}e^{\i (y_2\mu_\al+(x_1-y_1)\xi)}.
\een
By Cauchy integral theorem we have
\ben
\frac 1{2\pi}\int_{L^\eps_{-\phi}}\mathcal{N}(t)\cos t\,e^{\i \lam\cos t}dt
=\frac 1{2\pi}\int_{L}\mathcal{N}(t)\cos t\,e^{\i \lam\cos t}dt.
\een
...
\begin{figure}
	\centering
	\includegraphics[width=0.8\textwidth,height=0.5\textwidth]{./graphic/transformation5.png}
...
{\rm 2}. For all $k\geq2$, if $|u^{(k)}(t)|\ge 1$ for $t\in (a,b)$ , then for any $\phi(t)$ in $(a,b)$ with integrable derivatives
	\ben
	\left|\int^b_a e^{\i\lambda u(t)}\phi(t)dt\right|\le
	Ck\lambda^{-1/k}\left[|\phi(b)|+\int^b_a|\phi'(t)|dt\right].
	\een
\end{lem}
...
Assume that $\lam\ge 1$ and  $\phi\in (-\pi/2,\pi/2)$, Let $f(t)$ be continuous and have absolutely  integrable derivative in $(-\pi/2,\pi/2)$. Then for any $(a,b)\subset (-\pi/2,\pi/2)$, we have
 \be\label{c1}
   \left|\int_{-\frac\pi 2}^{\frac\pi 2}f(t)e^{\i\lam\cos t}dt\right| 
...
Let $x\in\Gamma_0$, $y\in\R_+^2$ satisfy $|x_1-y_1|/|x-y|>(1+\kappa)/2$ and $k_sy_2\ge 1$. There exists a constant $C$ depending only on $\kappa$ such that
	\ben
	|\N(x,y)|\leq \frac{C}{\mu}\left(\frac{k_sy_2}{(k_s|x-y|)^{3/2}}+e^{-\sqrt{k_R^2-k_s^2}y_2}\right).
	\een
\end{thm}

\debproof The starting point is (\ref{h3}) in Lemma \ref{lem:2.3}. Without loss of generality, we assume $x_1>y_1$ and thus $\phi\in (0,\pi/2)$. It is easy to see from Lemma \ref{deltaL} that the second term in (\ref{h3}) is bounded by $C\mu^{-1}e^{-\sqrt{k_R^2-k_s^2}y_2}$. 
...
\een
We shall only estimate the term involving $[\Np(k_s\sin(t+\phi))]_{22}=\mu^{-1}[\beta\mu_p](k_s\sin(t+\phi))$. The other elements can be proved similarly. Thus denote by
\ben
...
\ben
\left|\int_{-\frac\pi 2}^{\frac \pi 2}g(t)\cos te^{\i\lam\cos t}dt\right|\le C\lam^{-1/2}\left[|g(0)|+\int^{\frac\pi 2}_{-\frac\pi 2}|g'(t)|dt\right]\le C\mu^{-1}\lam^{-1/2}.
\een
Since $\de(\xi)$ is bounded below by $Ck_s^2|\xi|^2$ in then hyperbola $H$, we conclude from Lemma \ref{lem:2.7} that
\ben
\left|\int_{L\bks [-\frac\pi 2,\frac\pi 2]}g(t)\cos te^{\i\lam\cos t}dt\right|\le C\mu^{-1}\lam^{-1}.
\een
Thus $|{\rm I}_1|\le C\mu^{-1}\lam^{-1/2}\cos\phi$. Similarly, we can show $|{\rm I}_2|\le C\mu^{-1}\lam^{-3/2}$. The only difference is that since $g'(t)=f'(t)(\kappa^2-\sin^2(t+\phi))^{1/2}-f(t)(\kappa^2-\sin^2(t+\phi))^{-1/2}\sin(t+\phi)\cos(t+\phi)$, one has to use both (\ref{c1}) and (\ref{c3}) in Lemma \ref{lem:2.5} to obtain
\ben
...
\be
& & \De_e [\D(x,y)q] + \omega^2 [\D(x,y)q] = -\mathbf{\de}_y(x)q \mathbb{I} \ \ \ \ \mbox{in} \ \ \  \R^2_+ , \label{eq_d1} \\
...
\be\label{DGT1}
\D(x,y)=\G(x,y)-\G(x,y')+\frac{1}{2\pi}\int_{\R}\hat\D_c(\xi,x_2;y_2)e^{\i(x_1-y_1)\xi}d\xi,
\ee
where
\ben
\hat\D_c(\xi,x_2;y_2)=\frac{\i}{\om^2\ga(\xi)}\sum_{\al,\beta=s,p}\mathbb{B}_{\al\beta}(\xi)e^{\i(x_2\mu_\alpha+y_2\mu_\beta)}.
\een
...
\be\label{j}
\hskip-1.5cm[\J(x,y)]_{ij}:=e_i\cdot[\J(x,y)e_j]=\int_{\Ga_0}\T_D(x,z)e_i\cdot\overline{\N(x,y)}e_jds(x), \ \ i,j=1,2.
\ee
By the limiting absorption principle, we know that
\ben\hspace{-1cm}
[\J(z,y)]_{ij}=\lim_{\ep\to 0^+}\int_{\Ga_0} [\sigma(\D_{\om(1+\i\ep)}(x,z)e_i)e_2]\cdot[
...
Let the obstacle $D$ is included in the imaging domain $\Omega$ and $h=\dist(\Omega,\Gamma_0)$ be the distance between $\Omega$ and $\Gamma_0$. We assume  there exist constants $0<c_1<1,c_2>0,c_3>0$ such that
\be\label{d0}
...
The aim of this section is to show that for $z,y\in\Om$, $\F(z,y)$ is the main contribution in $\J_d(z,y)$ and $\F(z,y)$ decays as $|z-y|\to\infty$ and the imaginary part of the function $|\Im\J_{ii}(z,y)|, i=1,2$, peaks when $z=y$.
...
The following term shows the second term on the right-hand side of (\ref{d3}) is small.
...
\ben
\left|\int^\kappa_{-\kappa}f(t)e^{\i\lam\phi(t)}dt\right|\leq C\lambda^{-\frac{1}{4}}\left(|f(\kappa)|+\int_{-\kappa}^{\kappa}|f'(t)|dt\right).
\een
\end{lem}
\debproof
...
\ \ \zeta_4=\left[\tau\kappa^2\,\frac{\kappa^2+4t^2}{1+4t^2}\right]^{3/7},\ \ \zeta_5=\left[\tau\kappa^2\,\frac{3\kappa^2+4t^2}{3+4t^2}\right]^{1/3}.
\een
...
\ben
|\J(z,y)-\F(z,y)|+k_s^{-1}|\na_y(\J(z,y)-\F(z,y))|\leq \frac{C}{\mu}(k_s h)^{-\frac 14}.
\een
\end{thm}
...
\ben
\hskip-1cm{\rm II}:&=&\frac 1{2\pi}\sum_{\al,\beta=p,s}\int_{-k_s}^{k_s}\frac{{\Ta}(\xi)^T \overline{\Nb}(\xi)}{\overline{\delta(\xi)}} e^{\i (\mu_\alpha z_2-\overline{\mu}_\beta y_2)+\i(y_1-z_1)\xi} d\xi-\F(z,y)\\
...
\ben
\left|\int_{-k_p}^{k_p}\frac{\Tp(\xi)\overline{\Ns(\xi)}}{\overline{\de(\xi)}}e^{\i(\mu_py_2-\mu_s z_2)}d\xi\right|\le\frac{C}{\mu}(k_sh)^{-\frac 14}.
\een
...
and
	...
	\ee
	Then there exits a constant C depending only on $\kappa$ but independant of $k_s, h,d_D$ such that
	\ben\hskip-2cm
	\|\sigma(u_1-u_2)\nu\|_{H^{-1/2}(\Gamma_D)}
	\le\frac{C}{\mu}(1+\|T_1\|)(1+\|T_2\|)(1+k_s d_D)^2(k_sh)^{-1/2}\|g\|_{ H^{1/2}(\Ga_D)}.
	\een
Here $T_1, T_2:H^{1/2}(\Ga_D)\to H^{-1/2}(\Ga_D)$ are the Dirichlet to Neumann mapping associated to the elastic scattering problem (\ref{e1}) and (\ref{e2}), respectively. $\|T_1\|, \|T_2\|$ denote their operator norms.
\end{thm}
...
\ben
\hat{\V}(z,x_s)\cdot e_j=\GG(\overline{u^s_q(\cdot,x_s)},\left[\int_{\Ga_0^d}\sum^2_{i=1}[\T_D(x_r,z)]_{ij}\overline{\N(\cdot,x_r)}e_ids(x_r)\right]\,).
\een
...
\be
\hskip-1cm\overline{\W(z,x)}e_j=-\int_{\Ga_0^d}\sum^2_{k=1}[\T_D(x_s,z)]_{kj}\overline{\N(x,x_s)}e_kds(x_s)
...
\ben
|{\rm VI}_1|\le\frac C{\mu^2}(1+\|T_1\|)(1+k_sd_D)^2\left[\left(\frac hd\right)^{2}+(k_sh)^{-1/4}\right].
\een
...
\een
where $\GG(\cdot,\cdot)$ is defined in (\ref{g1}) and $\G(\cdot,\cdot)$ is the fundamental solution matrix of the elastic wave equation introduced in section 2. For any $x\in\Ga_0, z\in\R^2$, denote by $\T(z,x)\in\C^{2\times 2}$ be the traction tensor, $\T(z,x)q=\sigma(\G(z,x)q)e_2,\forall q\in\R^2$. The $(i,j)$-th element of $\T(z,x)$ is 
\ben
...
\ben \hspace{-1cm}
\hat{\T}(z_2;\xi,0)&=&\frac 12\Bigg( \begin{array}{cc}
	1 & \frac{\xi}{\mu_s} \\
	2(k_p^{-2}-k_s^{-2})\xi\mu_s & 2(k_p^{-2}-k_s^{-2})\xi^2
\end{array} \Bigg)e^{\i\mu_s z_2}\\
&+&\frac{1}{2}
\Bigg(\begin{array}{cc}
	0 & 0 \\
	-[1-2(k_p^{-2}-k_s^{-2})\xi^2]\frac{\xi}{\mu_p^2} & 1-2(k_p^{-2}-k_s^{-2})\xi^2 
...
\ben
\hskip-1cm\V(z,y)&=&\frac 1{2\pi}\sum_{\al,\beta=p,s}{\rm p.v.}\int_{\R}\frac{\tilde{\T}_{\alpha}(\xi)^T\N_\beta(\xi)}{\de(\xi)}e^{\i\mu_\al z_2+\i\mu_\beta y_2-\i(y_1+z_1)}d\xi\\
\hskip-1cm& &-\frac\i 2\sum_{\al,\beta=p,s}\left[\frac{\tilde{\T}_\alpha(\xi)^T\N_\beta(\xi)}{\de'(\xi)}e^{\i\mu_\al z_2+\i\mu_\beta y_2-\i\xi(y_1+z_1)}\right]^{k_R}_{-k_R}:={\rm V}_1+{\rm V}_2.
\een
