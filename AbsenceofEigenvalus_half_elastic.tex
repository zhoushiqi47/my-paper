...
& & (\lambda+\mu)\frac{d\hat u_2}{dx_2}+\i\lambda\xi\hat u_1 = 0 \label{pp6}
\ee
In order to work wth real coefficient, we use the change of variables:
\ben
v_1=\i u_1 , \ \ \ \  v_2=u_2,  \ \ \ \  \v=(v_2,v_2)^T
\een
\ben
[\A_1 \frac{d^2}{dx_2^2} +(\A_\xi^2-(\A_\xi^2)^T)\frac{d}{dx_2}-\A^3]\v
\een
\ben \hspace{-2.5cm}
\A^1=\Bigg( \begin{array}{cc}
\mu &  0\\
0 & \lambda+2\mu
\end{array} \Bigg) ,\ \  \ \ \
\A_\xi^2=\Bigg( \begin{array}{cc}
 0 & -(\lambda+\mu)\xi\\
(\lambda+\mu)\xi & 0
\end{array} \Bigg), \ \ \ \
\A^3=\Bigg( \begin{array}{cc}
	\lambda+2\mu &  0\\
	0 & \mu
\end{array} \Bigg)
\een
