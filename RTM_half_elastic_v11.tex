...
\end{lem}
\debproof
The estimate (\ref{c1}) follows directly from Lemma \ref{van} since the interval $(a,b)$ can be divided into several subintervals so that in each subinterval, either $|\cos t|\ge 1/\sqrt 2$ or $|\sin t|\ge 1/\sqrt 2$ and
$-\sin t$ is monotone. 
...
In the remainder of this paper we denote $\kappa=k_p/k_s\in (0,1)$. The following lemma collects some facts about the Rayleigh function $\de(\xi)$.
...
\een
where $\T_D^{\,\omega(1+\i\eps)}(x,z)q=\sigma(\D_{\om(1+\i\eps)}(x,z)q)e_2,\forall q\in\R^2$.
...
We remark that Theorem \ref{thm:4.3} indicates for extended obstacles, that is, when $k_sd_D\approx 1$, the error $R_d(z)$ in the imaging function is small when the obstacle is far away from the surface $k_sh\gg 1$ and the aperture is large $d\gg h$. Thus the first term on the right-hand side of (\ref{id}) dominates in the imaging function $\hat I_d(z)$.
...
field by considering the boundary at $x\in\Ga_D$ locally as a line with the normal $\nu(x)$. Let $S(x)\in\R^{2\times 2}$ be the rotation matrix such that $S(x)\nu(x)=(0,1)^T$. By using the analytical representation for the incident plane wave over the line \cite[P. 172]{achenbach1980}, the total field of (\ref{sc1}) with the incident $p$-wave is
\ben
...
\een
Here $\kappa(x)$ is the curvature of $\Ga_D$. Now for $z$ in the part of $\Ga_D$ which is back to $\Ga_0$, i.e., $\nu(z)\cdot\tau(\theta)>0$ for any $\theta\in (0,\pi)$, we know that $z$ and $x_-(\theta)$ are far away and thus $\hat{I}_d(z)\approx0$. This indicates that one cannot image the back part of the obstacle with only the data collected on $\Ga_0$. This is confirmed in our numerical examples in section 6.

On the other hand, if $z$ is in the illuminating part of $\Ga_D$ where $\nu(z)\cdot\tau(\theta)<0$ for some $\theta\in (0,\pi)$, by $\nu(x_-(\theta))=-\tau(\theta)$ we know  from (\ref{sc2})-(\ref{sc3}) that
\ben
R_p(x_-(\theta);\tau(\theta))=-2(\lam+2\mu)\tau(\theta),\ \ R_s(x_-(\theta);\tau(\theta))=-2\mu\tau(\theta)^\perp.
\een
This implies that $\hat I_d(z)$ is the weighted sum of $[\kappa(x_-(\theta))]^{-1/2}$ for $x_-(\theta)$ close to $z$ on $\Ga_D$ due to the property of $\F(z,x_-(\theta))$ in Theorem \ref{thm:3.2}.
...
\begin{thebibliography}{99}
	...
	\bibitem{chang1987}
	Chang W-F and McMechan G A 1987 {Elastic reverse-time migration} {\it Geophysical Prospecting} {\bf 37} 
	243 -- 256 
	...
	\bibitem{RTMhalf_aco}
	Chen Z and Huang G 2015 {Reverse time migration for reconstructing extended obstacles in the half space}  {\it Inverse Problems} {\bf 31 }  055007 (19pp)
	...
	{\it SAIM: Math. Model. Numer. Anal.} {\bf 44} 671-692
