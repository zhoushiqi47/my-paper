...
\begin{lem}\label{es_ngreen}
	For every $x\in\Gamma_0$, $y\in\R_+^2$ that $|x_1-y_1|/|x-y|>(1+\kappa)/2$, $y_2/|x-y|<\kappa/2$ and $k_s |x-y|>1$, we have
	\be\hspace{-2.5cm}
	|N(x,y)|\leq \frac{C}{\mu}\Bigg(\frac{k_s y_2}{|x-y|}\frac{1}{(k_s|x-y|)^{1/2}}+\frac{k_s |x_1-y_1|}{|x-y|}\frac{1}{(k_s|x-y|)^{3/2}}+e^{-\sqrt{k_R^2-k_s^2}y_2}\Bigg)
	\ee
	where C is only dependent on $\kappa$.
\end{lem}
...
\een
In order to analysis the PSF, loss is assumed in the medium that $k_{\alpha,\varepsilon}:=k_\alpha(1+\i\varepsilon)$. Then by Parseval identity and lemma \ref{root_De2}, we carry out
\ben\hspace{-1.5cm}
...
\ben\hspace{-1.5cm}
J^{\alpha\eta}(z,y)&=&\lim_{\varepsilon\to 0^+} \ \int_R \  ({T}^\alpha_D (x_1,0;z_1,z_2))^T \overline{{N}^{p,\varepsilon} (x_1,0;y_1,y_2)} dx_1 \\
&=&\lim_{\varepsilon\to 0^+} \ \frac{1}{2\pi}\int_R \  (\hat{T}^\alpha_D (\xi,0;z))^T \overline{\hat{N}^{\eta,\varepsilon} (\xi,0;y)} d\xi \\
&=& \frac{1}{2\pi}\int^{k_p}_{-k_p} \  (\hat{T}^s_D (\xi,0;z))^T \overline{\hat{N}^{\eta,\varepsilon} (\xi,0;y)} d\xi \\
&+&\lim_{\varepsilon\to 0^+} \ \frac{1}{2\pi}\int_{R\bks[-k_p,k_p]} \  (\hat{T}^\alpha_D (\xi,0;z))^T \overline{\hat{N}^{\eta,\varepsilon} (\xi,0;y)} d\xi \\
...
and for $(\alpha,\eta)\neq(s,s)$
\ben\hspace{-1.5cm}
\overline{R^{\alpha\eta}(y,z)}&=&\lim_{\varepsilon\to 0^+} \ \frac{1}{2\pi}\int_{R\bks[-k_s,k_s]} \  \overline{(\hat{T}^\alpha_D (\xi,0;z))^T} \hat{N}^{\eta,\varepsilon} (\xi,0;y) d\xi \\
&=&\lim_{\varepsilon\to 0^+} \ \frac{1}{2\pi}\int_{R\bks[-k_p,k_p]} \  (\Theta^\alpha (\xi;z))^T \hat{N}^{\eta,\varepsilon} (\xi,0;y) d\xi  \\
...
\begin{lem} \label{r_estimate2}
	...
	|{\MyRoman{2}}^{ps}_{ij}(x,y)|\le\frac{C}{\mu}e^{-\sqrt{k_R^2-k_p^2}z_2+\sqrt{k_R^2-k_s^2}y_2}
	\ee
...
\begin{lem} \label{r_estimate3}
	...
	|{\MyRoman{3}}^{ps}_{ij}(x,y)|\le\frac{C}{\mu k_s z_2}
	\ee
	where $C$ is only dependent on $\kappa$.
\end{lem}
\debproof
...
\begin{lem} \label{r_estimate4}
	For any $z,y\in \Om$,
	\be
	|F^{sp}_{ij}(x,y)|\le\frac{C}{\mu }\frac{1}{(k_s h)^{\frac{1}{2n^*}}} \ \ \
	|F^{ps}_{ij}(x,y)|\le\frac{C}{\mu }\frac{1}{(k_s h)^{\frac{1}{2n^*}}}
	\ee
	where $C$ is only dependent on $\kappa$.
\end{lem}
\debproof
...
\begin{cor}\label{cor_dpsf}
	...
	\leq \frac{C}{\mu} ((\frac{h}{d})^{2}+\frac{(k_s h)^{1/2}}{ e^{\sqrt{k_r^2-k_s^2}h}}(\frac{h}{d})^{1/2})(1+k_sd_D)
	\een
	uniformly for $z\in\Om$, where $d_D$ is the diameter of the obstacle D.	
\end{cor}
...
Then, we have
\be
\hat{I}_d(z)=\Im\mathbf{tr}\{\int_{\Ga_D} [T^{\nu}_y(\overline{F(z,y)}+\Phi(y,z)]\overline{F(z,y)}ds(y)\}+W_{\hat{I}}(z)
\ee
