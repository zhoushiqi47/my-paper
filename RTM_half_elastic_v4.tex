...
\begin{figure}
	\centering
	\includegraphics[width=0.8\textwidth]{./graphic/transfor.png}
...
\begin{thm} \label{elastic_eq2}
	...
	\ee
\end{thm}
Moreover, we give the definition of the Dirichlet-to-Neumann mapping for Elastic scattering problems. For any $g\in H^{1/2}(\Ga_D) $, define $T_h(g)= \sigma(u)\nu\in H^{-1/2}(\Ga_D)$ where  u is the corresponding solution of above equation and also $T_f(g)=\sigma(v)\nu$ where v is the solution of full space scattering proble. we will denote $\|T_h\|$ or $\|T_f\|$ by its operator norm in the remainder of this paper. 
...
\section{Some proof; Name of Section TBD}
...
\ben
|\pv \int_{1}^{\infty}\frac{k_s^4 g(t)}{\delta(k_s t)}|\leq C\Big( \int_{1}^{\infty}|g(t)|dt+ \max_{t\in[1,2\kappa_R-1]}(|g(t)|+|g'(t)|)\Big)
\een
...
\begin{thm}\label{resolution2}
	...
	\een
	where $|W_{\hat{I}}(z)|\leq C(1+\|T_f\|)(1+\|T_h\|)(1+k_s d_D)^4(\epsilon_1(k_s h)+\epsilon_2(k_s h,h/d))$ uniformly for z in $\Om$.
\end{thm}
