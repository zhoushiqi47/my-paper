...
Therefore, the estimate of $T_D(x_1,0;y_1,y_2)$ and $N(x_1,0;y_1,y_2)$ are now direct consequence of lemma \ref{singular_term}.
...
\ee
Similarly, we have $|I_{21}+I_{22}|\leq 1/(k_s\rho)^{1/2}$ by lemma \ref{singular_term}.
...
\ee
Let $ V(y,z):=\overline{ W(y,z)}-W_d(y,z)-\Phi(y,z)$. Since $\overline{ W(y,z)}-W_d(y,z)$ satisfy the half-space scattering problem \ref{elas_r1} with $g(y)=-\overline{F(z,y)}$, by using Theorem \ref{diff_solu} and Corollary \ref{cor_psf}
\be
\|T_y^{\nu}V(y,z)\|_{H^{1/2}(\Gamma_D)}&\leq& C(1+k_s d_D)^2\epsilon_1(k_s h)\|F(z,\cdot)\|_{H^{1/2}(\Gamma_D)}\\
&\leq&C(1+k_s d_D)^3\epsilon_1(k_s h)
\ee
Now we substitute $\overline{ W(y,z)}=V(y,z)+W_d(y,z)+\Phi(y,z)$ into (\ref{resolu_1}) to obtain
...
By (\ref{F_p})-(\ref{F_s}) we know that for any fixed $z\in\Om$, $\overline{F(z,\cdot)}$ satisfies the Elastic wave equation. Thus $\Phi(y,z)$ can be viewed as the scattering solution of the Elastic equation with the
...
as illustrated 
\section{Appendix: The forward scattering problem}
