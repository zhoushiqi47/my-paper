...
\ee
The estimate in lemma \ref{es_dgreen}-\ref{es_ngreen} show that the integral above exists. We split the spectral terms into components associated with pressure and shearing waves.
\ben
...
\ben\hspace{-1.5cm}
\J^{ss}(z,y)&=&\lim_{\ep\to 0^+} \ \frac{1}{2\pi} \int_R \  ({\T}^s_D (x_1,0;z_1,z_2))^T \overline{{\N}^{s,\ep} (x_1,0;y_1,y_2)} dx_1 \\
&=& \frac{1}{2\pi}\int^{k_s}_{-k_s} \  \frac{{\Ts}(\xi)^T \overline{\Ns}(\xi)}{\overline{\delta(\xi)}} e^{\i (\mu_s z_2-\overline{\mu_s} y_2)+\i(y_1-z_1)\xi} d\xi  \\
&+&\lim_{\ep\to 0^+} \ \frac{1}{2\pi}\int_{R\bks[-k_s,k_s]} \  \frac{{\Ts^{\ep}}(\xi)^T \overline{\Ns^{\ep}}(\xi)}{\overline{\delta^{\ep}(\xi)}} e^{\i (\mu_s^{\ep} z_2-\overline{\mu_s^{\ep}} y_2)+\i(y_1-z_1)\xi} d\xi   \\
&:=&\F^{ss}(z,y)+\gR^{ss}(z,y)
\een
Similarly, we have
\ben\hspace{-1.5cm}
\J^{pp}(z,y)&=&\lim_{\ep\to 0^+} \ \frac{1}{2\pi} \int_R \  ({\T}^p_D (x_1,0;z_1,z_2))^T \overline{{\N}^{p,\ep} (x_1,0;y_1,y_2)} dx_1 \\
&=& \frac{1}{2\pi}\int^{k_s}_{-k_s} \  \frac{{\Tp}(\xi)^T \overline{\Np}(\xi)}{\overline{\delta(\xi)}} e^{\i (\mu_p z_2-\overline{\mu_p} y_2)+\i(y_1-z_1)\xi} d\xi  \\
&+&\lim_{\ep\to 0^+} \ \frac{1}{2\pi}\int_{R\bks[-k_s,k_s]} \  \frac{{\Tp^{\ep}}(\xi)^T \overline{\Np^{\ep}}(\xi)}{\overline{\delta^{\ep}(\xi)}} e^{\i (\mu_p^{\ep} z_2-\overline{\mu_p^{\ep}} y_2)+\i(y_1-z_1)\xi} d\xi   \\
&:=&\F^{pp}(z,y)+\gR^{pp}(z,y)
\een
 Our goal now is to show which is the main contribution to the point spread funciton when $k_s h\gg1$. Put $n_*=\min\{N|\kappa^{2N-1}<1/c_3,N\in \Z_+ \}$. Then we present some primary results for point spread function, whose proof will be stated in seciton 4.
...
To complete the analysis of the point spread function, Substitute (\ref{}) and (\ref{ngreen}) into  $F_{ss}(z,y),F_{pp}(z,y)$:
\be
