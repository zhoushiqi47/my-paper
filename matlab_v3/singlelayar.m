function A=singlelayar(omega, lambda, mu, n, bctype)
    ...
    for j=1:2*n
        for k=1:2*n
            if (j==k)
                ...
                H{1,1}(j,k) = 2*alpha*log(dist) + ka1 + ka2*dx1(j)*dx1(j)/dist^2+G(1,1);
                H{2,2}(j,k) = 2*alpha*log(dist) + ka1 + ka2*dx2(j)*dx2(j)/dist^2+G(2,2);
            
                H{1,2}(j,k) =                           ka2*dx1(j)*dx2(j)/dist^2+G(1,2);
                H{2,1}(j,k) =                           ka2*dx2(j)*dx1(j)/dist^2+G(2,1);
                for l=1:2
                    for m=1:2
