function Data = NyElasticWave_src_half_symm(omegas, lambda, mu, n, bctype, n_src, n_recv, source, receiver)
%% the exact solution is given by the radiation solution
%% 2016 07 22 by zhou
%% modified 2017 12 8
%% every matrix is symmtry 
...
for kk=1:Nomegas
    ...
    yl=[];
    for j=1:2*n
        xl=[xl,repmat([x1(j);x2(j)],1,1+2*n-j)];
        yl=[yl,[x1(j:2*n),x2(j:2*n)]'];
    end
    rGG  = Elastic_GreenTensor_Thalf_SIP1(omega,kp,ks,yl,xl);
   %只计算 上对角矩阵
   jl=1;
    for j=1:2*n
        for k=j:2*n
