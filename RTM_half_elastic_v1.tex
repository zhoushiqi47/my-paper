...
&=& \ \frac{1}{2\pi}\int_{\Gamma^\pm_l \cup \Gamma^\pm_r} \  (\Theta^s (\xi;z))^T \hat{N}^{s} (\xi,0;y) d\xi \pm \\
& & \i \lim_{\xi\to k_R} (\xi-k_R)(\Theta^s (\xi;z))^T \hat{N}^{s} (\xi,0;y) \\
...
&=& \ \frac{1}{2\pi}\int_{\Gamma^\pm_l \cup \Gamma^\pm_r} \  (\Theta^\alpha (\xi;z))^T \hat{N}^{\eta} (\xi,0;y) d\xi \pm \\
& & \i \lim_{\xi\to k_R} (\xi-k_R)(\Theta^\alpha (\xi;z))^T \hat{N}^{\eta} (\xi,0;y)+ \\
& & \frac{1}{2\pi}\int_{(-k_s,-k_p)\cup(k_p,k_s)} \  \overline{(T^\alpha (\xi;z))^T} \hat{N}^{\eta} (\xi,0;y) d\xi \\
...
Moreover, 
\be\hspace{-2cm}
|R^{ij}(z,y)|+k_s^{-1}|\na_y R^{ij}(z,y)|\leq \frac{C}{\mu}(\frac{1}{(k_s h)^{\frac{1}{2n^*}}}+e^{-k_s h\sqrt{\kappa_R^2-1}}):=\frac{C}{\mu} \epsilon_1(k_s h)
\ee
...
\overline{W(y,z)}&=&\sum_{k=1}^{2}\int_{\Gamma_0^d}
 ...
 &=&-\overline{J_d^T(z,y)}
 \een
 Moreover, $T_y^{e_2}\overline{W_k(y,z)}=0$ on $\Gamma_0$ since $T_y^{e_2}u^s_k(y,x_s)=0$ on $\Gamma_0$. Let $W_d(y,z)=\overline{W(y,z)}-\Psi(y,z)$ and it follows that $W_d(y,z)$ is the scattering solution of the problem
 \be
...
\be
|R_{\hat{I}}(z)|\leq C(1+k_s d_D)^2(\epsilon_1(k_s h)+\epsilon_2(k_s h,h/d))
\ee
...
Now, by the theorem \ref{diff_solu},\ref{resolution1}, we have more simpler theorem: 
\begin{thm}\label{resolution2}
	For any $z\in\Omega$, let $\Phi(y,z)\in\C^{2\times2}$ be the radiation solution of the problem:
	\ben
	& & \Delta_e \Phi(y,z) + \omega^2\Phi= 0 \ \ \ \ \mbox{in }\R^2\bks \bar{D}\\
	& &\Phi(y,z)= -\overline{F(z,y)} \ \ \ \ \ \ \ \mbox{on} \ \Ga_D  \\ 
	\een
	Then, we have
	\be\hspace{-1cm}
	\hat{I}_d(z)=\Im\mathbf{tr}\int_{\Gamma_D}(T^{\nu}_y (\overline{F(z,y)}+\Phi(y,z))^T \overline{F(z,y)}ds(y)+W_{\hat{I}}(z)
	\ee
...
\end{thm}
By (\ref{F_p})-(\ref{F_s}) we know that for any fixed $z\in\Om$, $\overline{F^T(z,\cdot)}$ satisfies the Elastic wave equation. Thus $\Phi(y,z)$ can be viewed as the scattering solution of the Elastic equation with the
...
\een
By the same procedure that used in \cite[p11,12]{RTMhalf_aco}, we can carry out that 
\ben\hspace{-1cm}
...
\begin{thm}\label{resolution2}
	...
	\ben
	& & \Delta_e \Phi(y,z) + \omega^2\Phi= 0 \ \ \ \ \mbox{in }\R^2\bks \bar{D}\\
	& &T^{\nu}_y\Phi(y,z)+\i\eta(y)\Phi(y,z)= -(T^{\nu}_y+\i\eta(y))\overline{F(z,y)} \ \ \ \ \ \ \ \mbox{on} \ \Ga_D  \\ 
	\een
	Then, we have
	\be\hspace{-1cm}
...
\ee
where $n(x)\in L^{\infty}_{\R^2}$ is a positive function which is equel to 1 outside $D$. By modifying the argument in Theorem \ref{resolution2}, the following theorem can be proved.
\begin{thm}\label{resolution2}
	For any $z\in\Omega$, let $\Phi(y,z)\in\C^{2\times2}$ be the radiation solution of the problem:
	\ben
	& & \Delta_e \Phi(y,z) + \omega^2n(y)\Phi= -\omega^2(n(y)-1)\overline{F(z,y)} \ \ \ \ \mbox{in }\R^2
	\een
	Then, we have
	\be\hspace{-1cm}
...
peanut along the vertical direction. The angular frequency is $\om=3\pi$ for the test of the single frequency and $\om=\pi\times[2:0.5:8]$ for the test of multiple frequencies. Figure \ref{figure_3} shows the imaging result of the first model. The
...
\begin{figure}
	...
	\caption{Example 3:From left to right,  the imaging result
		with single frequency data where $\om=2\pi,4\pi$, the imaging result with multiple frequency data.}\label{figure_3}
\end{figure}
